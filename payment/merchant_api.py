...
from payment.serializers import PaycomOperationSerializer
from payment.authentication import authentication
from .status import *
...
class MerchantAPIView(APIView):
    ...
    @swagger_auto_schema(request_body=PaycomOperationSerializer)
    def post(self, request):
        print("#"*50, "received data from payment swagger or sendbox")
        print(f"request data : {request.data}")
        check = authentication(request)
        if check is False or not check:
            return Response(AUTH_ERROR)
        print("#"*20)
        print("registratsiyadan o'tdi")
        serializer = PaycomOperationSerializer(data=request.data, many=False)
